...
public class ReciprocalArraySumTest extends TestCase {
    ...
    private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
        ...
        if (useManyTaskVersion) {
            sum = ReciprocalArraySum1.parManyTaskArraySum(input, ntasks);
        } else {
            assert ntasks == 2;
            sum = ReciprocalArraySum1.parArraySum(input);
        }
        ...
        for (int r = 0; r < REPEATS; r++) {
            if (useManyTaskVersion) {
                ReciprocalArraySum1.parManyTaskArraySum(input, ntasks);
            } else {
                assert ntasks == 2;
                ReciprocalArraySum1.parArraySum(input);
            }
        }
