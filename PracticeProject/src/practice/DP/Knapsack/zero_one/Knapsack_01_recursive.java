...
public class Knapsack_01_recursive {
    ...
    /* Important things for consideration while writing a recursive code :
    * 1. Base condition
    * 2. Choice Diagram
    * 3. Think of the smallest VALID input
    * */
    ...
    static int knapsack(int[] value, int[] weight, int n, int capacity) {
        ...
        if (weight[n - 1] <= capacity) {
            return Math.max((value[n - 1] + knapsack(value, weight, n - 1, capacity - weight[n - 1])), // pick element
                    knapsack(value, weight, n - 1, capacity)); // don't pick this element
        }
