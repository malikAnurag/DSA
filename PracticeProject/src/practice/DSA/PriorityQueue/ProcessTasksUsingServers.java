 ...
 * As long as there are free servers and the queue is not empty, the task in the front of the queue will be assigned to a free server with the smallest weight, and in case of a tie,
...
public class ProcessTasksUsingServers {
    ...
    public int[] assignTasks(int[] servers, int[] tasks) {
        ...
        PriorityQueue<int[]> available = new PriorityQueue<int[]>((a, b) -> a[0] != b[0] ? a[0] - b[0] : a[1] - b[1]); // weight, idx
        PriorityQueue<int[]> busy = new PriorityQueue<int[]>((a,b) ->
                a[0] != b[0] ? a[0] - b[0] : (a[1] != b[1] ? a[1] - b[1] : a[2] - b[2]));// [freeTime, weight, index]
        ...
        for(int i = 0 ; i < tasks.length ; i++) {
            ...
            ans[idx++] = server[1];
            busy.add(new int[]{time + tasks[i], server[0], server[1]});
        }
