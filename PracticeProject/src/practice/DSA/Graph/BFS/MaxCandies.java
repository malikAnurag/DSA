...
public class MaxCandies {
    ...
    public int maxCandies(int[] status, int[] candies, int[][] keys, int[][] containedBoxes, int[] initialBoxes) {
        ...
        int n = status.length;
        // state tracking
        boolean[] canOpen = new boolean[n]; // whether we have the key (or it's already open)
        boolean[] hasBox = new boolean[n]; // whether we possess the box
        boolean[] used = new boolean[n]; // whether we've already opened this box
        ...
        int ans = 0;
        // to explore each open box that we have
        Queue<Integer> q = new LinkedList<>();

        for (int box : initialBoxes) { // handle initial boxes : if open, collect candies, add to queue
            hasBox[box] = true;
            if (canOpen[box]) {
                q.offer(box);
                used[box] = true;
        ...
        while (!q.isEmpty()) {
            ...
            // collect candies from the big box
