...
//                 15   7
    ...
    public static int findMaxSumUtil(TreeNode root, Res res) {
        ...
        int left = findMaxSumUtil(root.left, res);
        int right = findMaxSumUtil(root.right, res);

        int maxSingle = Math.max(Math.max(left, right) + root.key, root.key);

        int maxTop = Math.max(root.key + left + right, maxSingle);
